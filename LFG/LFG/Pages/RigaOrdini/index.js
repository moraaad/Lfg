$(function () {
    ...
    var rigaOrdineService = window.lFG.rigaOrdini.rigaOrdini;
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'RigaOrdini/CreateModal',
        scriptUrl: abp.appPath + 'Pages/RigaOrdini/createModal.js',
        modalClass: 'rigaOrdineCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'RigaOrdini/EditModal',
        scriptUrl: abp.appPath + 'Pages/RigaOrdini/editModal.js',
        modalClass: 'rigaOrdineEdit',
    });

    var getFilter = function () {
        return {
            filterText: $('#FilterText').val(),
            quantitaMin: $('#QuantitaFilterMin').val(),
            quantitaMax: $('#QuantitaFilterMax').val(),
            prezzoUnitarioMin: $('#PrezzoUnitarioFilterMin').val(),
            prezzoUnitarioMax: $('#PrezzoUnitarioFilterMax').val(),
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.RigaOrdini.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.rigaOrdine.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.RigaOrdini.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            rigaOrdineService.delete(data.record.rigaOrdine.id).then(function () {
        ...
        { data: 'rigaOrdine.quantita' },
        { data: 'rigaOrdine.prezzoUnitario' },
        {
    ...
    if (abp.auth.isGranted('LFG.RigaOrdini.Delete')) {
    ...
    var dataTable = $('#RigaOrdiniTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(rigaOrdineService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        rigaOrdineService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            rigaOrdineService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewRigaOrdineButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        rigaOrdineService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/riga-ordini/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'quantitaMin', value: input.quantitaMin },
                    { name: 'quantitaMax', value: input.quantitaMax },
                    { name: 'prezzoUnitarioMin', value: input.prezzoUnitarioMin },
                    { name: 'prezzoUnitarioMax', value: input.prezzoUnitarioMax },
