$(function () {
    ...
    var categoriaService = window.lFG.categorie.categorie;

    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Categorie/CreateModal',
        scriptUrl: abp.appPath + 'Pages/Categorie/createModal.js',
        modalClass: 'categoriaCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Categorie/EditModal',
        scriptUrl: abp.appPath + 'Pages/Categorie/editModal.js',
        modalClass: 'categoriaEdit',
    });
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.Categorie.Edit'),
                    ...
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.Categorie.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            categoriaService.delete(data.record.id).then(function () {
    ...
    if (abp.auth.isGranted('LFG.Categorie.Delete')) {
    ...
    var dataTable = $('#CategorieTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(categoriaService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        categoriaService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            categoriaService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        categoriaService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/categorie/as-excel-file' +
