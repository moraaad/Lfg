$(function () {
    ...
    var elementoListaService = window.lFG.elementoListe.elementoListe;
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ElementoListe/CreateModal',
        scriptUrl: abp.appPath + 'Pages/ElementoListe/createModal.js',
        modalClass: 'elementoListaCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ElementoListe/EditModal',
        scriptUrl: abp.appPath + 'Pages/ElementoListe/editModal.js',
        modalClass: 'elementoListaEdit',
    });

    var getFilter = function () {
        return {
            filterText: $('#FilterText').val(),
            dataAggiuntaMin: $('#DataAggiuntaFilterMin').val(),
            dataAggiuntaMax: $('#DataAggiuntaFilterMax').val(),
            varianteProdottoId: $('#VarianteProdottoIdFilter').val(),
            listaDesideriId: $('#ListaDesideriIdFilter').val(),
        };
    };
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.ElementoListe.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.elementoLista.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.ElementoListe.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            elementoListaService
                                .delete(data.record.elementoLista.id)
                                .then(function () {
        ...
        {
            data: 'elementoLista.dataAggiunta',

            render: function (dataAggiunta) {
                if (!dataAggiunta) {
                    return '';
                }

                var date = Date.parse(dataAggiunta);
                return new Date(date).toLocaleDateString(abp.localization.currentCulture.name);
            },
        },
        {
            data: 'varianteProdotto.urlImmagine',
    ...
    if (abp.auth.isGranted('LFG.ElementoListe.Delete')) {
    ...
    var dataTable = $('#ElementoListeTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(elementoListaService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        elementoListaService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            elementoListaService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewElementoListaButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        elementoListaService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/elemento-liste/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'dataAggiuntaMin', value: input.dataAggiuntaMin },
                    { name: 'dataAggiuntaMax', value: input.dataAggiuntaMax },
                    { name: 'varianteProdottoId', value: input.varianteProdottoId },
                    { name: 'listaDesideriId', value: input.listaDesideriId },
                ]);
