$(function () {
    ...
    var immagineVarianteService = window.lFG.immagineVarianti.immagineVarianti;
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ImmagineVarianti/CreateModal',
        scriptUrl: abp.appPath + 'Pages/ImmagineVarianti/createModal.js',
        modalClass: 'immagineVarianteCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ImmagineVarianti/EditModal',
        scriptUrl: abp.appPath + 'Pages/ImmagineVarianti/editModal.js',
        modalClass: 'immagineVarianteEdit',
    });
    ...
    var getFilter = function () {
        return {
            ...
            url: $('#UrlFilter').val(),
            ordineMin: $('#OrdineFilterMin').val(),
            ordineMax: $('#OrdineFilterMax').val(),
            varianteProdottoId: $('#VarianteProdottoIdFilter').val(),
        };
    };
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.ImmagineVarianti.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.immagineVariante.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.ImmagineVarianti.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            immagineVarianteService
                                .delete(data.record.immagineVariante.id)
                                .then(function () {
        ...
        { data: 'immagineVariante.varianteProdottoId' },
        { data: 'immagineVariante.url' },
        { data: 'immagineVariante.ordine' },
        {
    ...
    if (abp.auth.isGranted('LFG.ImmagineVarianti.Delete')) {
    ...
    var dataTable = $('#ImmagineVariantiTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(immagineVarianteService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        immagineVarianteService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
    ...
    $('#NewImmagineVarianteButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        immagineVarianteService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/immagine-varianti/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'varianteProdottoId', value: input.varianteProdottoId },
                    { name: 'url', value: input.url },
                    { name: 'ordineMin', value: input.ordineMin },
                    { name: 'ordineMax', value: input.ordineMax },
                    { name: 'varianteProdottoId', value: input.varianteProdottoId },
                ]);
