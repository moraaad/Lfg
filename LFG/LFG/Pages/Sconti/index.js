$(function () {
    ...
    var scontoService = window.lFG.sconti.sconti;

    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Sconti/CreateModal',
        scriptUrl: abp.appPath + 'Pages/Sconti/createModal.js',
        modalClass: 'scontoCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Sconti/EditModal',
        scriptUrl: abp.appPath + 'Pages/Sconti/editModal.js',
        modalClass: 'scontoEdit',
    });

    var getFilter = function () {
        return {
            filterText: $('#FilterText').val(),
            codice: $('#CodiceFilter').val(),
            tipo: $('#TipoFilter').val(),
            valoreMin: $('#ValoreFilterMin').val(),
            valoreMax: $('#ValoreFilterMax').val(),
            limiteUtilizziMin: $('#LimiteUtilizziFilterMin').val(),
            limiteUtilizziMax: $('#LimiteUtilizziFilterMax').val(),
            validoDalMin: $('#ValidoDalFilterMin').val(),
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.Sconti.Edit'),
                    ...
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.Sconti.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            scontoService.delete(data.record.id).then(function () {
        ...
        { data: 'limiteUtilizzi' },
        {
            ...
            render: function (validoDal) {
                if (!validoDal) {
                    return '';
                }

                var date = Date.parse(validoDal);
        ...
        },
        {
            ...
            render: function (validoAl) {
                if (!validoAl) {
                    return '';
                }

                var date = Date.parse(validoAl);
                return new Date(date).toLocaleDateString(abp.localization.currentCulture.name);
            },
        },
    ];
    ...
    var dataTable = $('#ScontiTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(scontoService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        scontoService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            scontoService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewScontoButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        scontoService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/sconti/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'codice', value: input.codice },
                    { name: 'tipo', value: input.tipo },
                    { name: 'valoreMin', value: input.valoreMin },
                    { name: 'valoreMax', value: input.valoreMax },
                    { name: 'limiteUtilizziMin', value: input.limiteUtilizziMin },
                    { name: 'limiteUtilizziMax', value: input.limiteUtilizziMax },
                    { name: 'validoDalMin', value: input.validoDalMin },
