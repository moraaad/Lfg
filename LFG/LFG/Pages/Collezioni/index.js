$(function () {
    ...
    var collezioneService = window.lFG.collezioni.collezioni;

    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Collezioni/CreateModal',
        scriptUrl: abp.appPath + 'Pages/Collezioni/createModal.js',
        modalClass: 'collezioneCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Collezioni/EditModal',
        scriptUrl: abp.appPath + 'Pages/Collezioni/editModal.js',
        modalClass: 'collezioneEdit',
    });
    ...
    var getFilter = function () {
        return {
            ...
            nome: $('#NomeFilter').val(),
            stagione: $('#StagioneFilter').val(),
            annoMin: $('#AnnoFilterMin').val(),
            annoMax: $('#AnnoFilterMax').val(),
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.Collezioni.Edit'),
                    ...
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.Collezioni.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            collezioneService.delete(data.record.id).then(function () {
        ...
        { data: 'stagione' },
        {
            ...
            render: function (anno) {
                if (!anno) {
                    return '';
                }

                var date = Date.parse(anno);
    ...
    if (abp.auth.isGranted('LFG.Collezioni.Delete')) {
    ...
    var dataTable = $('#CollezioniTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(collezioneService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        collezioneService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            collezioneService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewCollezioneButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        collezioneService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/collezioni/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'nome', value: input.nome },
                    { name: 'stagione', value: input.stagione },
                    { name: 'annoMin', value: input.annoMin },
                    { name: 'annoMax', value: input.annoMax },
