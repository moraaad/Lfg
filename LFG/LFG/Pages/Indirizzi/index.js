$(function () {
    ...
    var indirizzoService = window.lFG.indirizzi.indirizzi;
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Indirizzi/CreateModal',
        scriptUrl: abp.appPath + 'Pages/Indirizzi/createModal.js',
        modalClass: 'indirizzoCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'Indirizzi/EditModal',
        scriptUrl: abp.appPath + 'Pages/Indirizzi/editModal.js',
        modalClass: 'indirizzoEdit',
    });

    var getFilter = function () {
        return {
            filterText: $('#FilterText').val(),
            paese: $('#PaeseFilter').val(),
            citta: $('#CittaFilter').val(),
            provincia: $('#ProvinciaFilter').val(),
            via: $('#ViaFilter').val(),
            cap: $('#CapFilter').val(),
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('LFG.Indirizzi.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.indirizzo.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('LFG.Indirizzi.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            indirizzoService.delete(data.record.indirizzo.id).then(function () {
    ...
    if (abp.auth.isGranted('LFG.Indirizzi.Delete')) {
    ...
    var dataTable = $('#IndirizziTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(indirizzoService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        indirizzoService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            indirizzoService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewIndirizzoButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        indirizzoService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/app/indirizzi/as-excel-file' +
                abp.utils.buildQueryString([
                    { name: 'downloadToken', value: result.token },
                    { name: 'filterText', value: input.filterText },
                    { name: 'paese', value: input.paese },
                    { name: 'citta', value: input.citta },
                    { name: 'provincia', value: input.provincia },
                    { name: 'via', value: input.via },
                    { name: 'cap', value: input.cap },
